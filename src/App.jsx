...
const SCHEDULE = [
  {
    ...
    events: [
      ...
      { time: "4:00 PM — 6:00 PM", title: "Explore. Drink. Gamble.", emoji: "🎰",
        location: "The Strip & LINQ Promenade",
        address: "3545 S Las Vegas Blvd, Las Vegas, NV 89109",
        desc: "You've got two hours of free reign. Hit the tables, post up at a bar, or explore the Strip. The LINQ Promenade is right there — High Roller, bars, food, all of it. This is your warmup. Don't go too crazy though — dinner is at 6 and you WILL need your energy for tonight.",
        note: "Pro tip: Have a light snack if you're hungry but save your appetite for Dick's." },
      { time: "6:00 PM — 8:00 PM", title: "Dinner @ Dick's Last Resort", emoji: "🍺", tag: "MANDATORY", tagType: "mandatory",
        location: "Dick's Last Resort — Excalibur Hotel & Casino",
        address: "3850 S Las Vegas Blvd, Las Vegas, NV 89109",
        desc: "If you don't know what Dick's is — it's the restaurant where the waiters roast you, put giant paper hats on your head with embarrassing stuff written on them, and treat you like garbage. On purpose. It's hilarious. This is a bachelor party so expect Cesar to get absolutely destroyed by the staff. Leave your feelings at the door. Come hungry, the food is actually solid — classic American bar food, buckets of beer, and nonstop chaos.",
        note: "Leave your feelings at the door. Cesar WILL get roasted. The rest of you probably will too." },
      { time: "8:00 PM — 9:45 PM", title: "Back to Hotel — Get Ready", emoji: "🪞",
        location: "The LINQ Hotel",
        address: "3535 S Las Vegas Blvd, Las Vegas, NV 89109",
        desc: "Head back to the hotel after Dick's, clean up, change, and get ready for the night. Dress to impress — no flip flops, no gym shorts. You've got almost two hours so there's no excuse to be late for the bus." },
  ...
  },
  {
    ...
    events: [
      ...
      { time: "Afternoon", title: "Sunday Free Time", emoji: "🤷", tag: "TO BE ANNOUNCED", tagType: "tba",
        location: "TBA",
        address: "Open to suggestions — drop ideas in the group chat",
        desc: "Big gap between pool and dinner. We're open to suggestions — if you've got an idea for something to do, throw it in the chat. Otherwise it's free time to recover, gamble, explore, or nap before the last supper." },
...
const css = `
  ...
  .htl-title {
    font-family: 'Cinzel', serif; font-weight: 700;
    font-size: 1rem; color: var(--ivory); margin-bottom: 0.4rem; line-height: 1.3;
  }
  .htl-location {
    font-family: 'Cinzel', serif; font-weight: 500;
  ...
  .ag-time-col { min-width: 120px; text-align: right; padding-top: 4px; flex-shrink: 0; }
  .ag-time {
    font-family: 'Cinzel Decorative', serif; font-weight: 700;
  ...
  .ag-title {
    font-family: 'Cinzel', serif; font-weight: 700;
    font-size: 1rem; color: var(--ivory); margin-bottom: 0.2rem;
  }
  ...
  /* CESAR GLOW */
  .r-groom-wrap { position: relative; }
  .r-groom-wrap .r-avatar {
    animation: groomPulse 2.5s ease-in-out infinite;
    box-shadow: 0 0 20px rgba(201,168,76,0.4), 0 0 40px rgba(201,168,76,0.2), 0 0 60px rgba(201,168,76,0.1);
  }
  .r-groom-crown {
    position: absolute; top: -18px; left: 50%; transform: translateX(-50%);
    font-size: 1.6rem; filter: drop-shadow(0 0 6px rgba(201,168,76,0.6));
    animation: crownFloat 3s ease-in-out infinite;
    z-index: 2;
  }
  @keyframes groomPulse {
    0%, 100% { box-shadow: 0 0 20px rgba(201,168,76,0.4), 0 0 40px rgba(201,168,76,0.2), 0 0 60px rgba(201,168,76,0.1); }
    50% { box-shadow: 0 0 30px rgba(201,168,76,0.6), 0 0 50px rgba(201,168,76,0.3), 0 0 80px rgba(201,168,76,0.15); }
  }
...
export default function App() {
  ...
  const [guests, setGuests] = useState(() => {
    try {
      const saved = localStorage.getItem("clr-guests");
  ...
  useEffect(() => {
    try { localStorage.setItem("clr-guests", JSON.stringify(guests)); } catch {}
  }, [guests]);
          ...
          <div className="roster-grid">
            {guests.map(g => (
              <div key={g.id} className="r-card" style={{
                ...
                {g.role === "THE GROOM" ? (
                  <div className="r-groom-wrap">
                    <div className="r-groom-crown">👑</div>
